...
- Status lines are generated by `GitDir.status_line()`, for example `main ✅`,
  `main ☑️ 1 behind`, `foo 🪦`, or `main ☑️ [+1] ✏️9 🗑️3 🆕1 ⌛2d`.
...
- `status_line()` accepts an operation `GitRunReport` and appends its rendered
  ...
  notes such as stale fetch age, cleanable local branches, and detached `HEAD`.
...
- In a workspace, skipped/problem repos are reported inline and do not fail or
  stop the whole run.
- Successful pull reports append a note on the same line, such as
  `was 5 behind` or `was up-to-date`; it is styled as report-note text.
